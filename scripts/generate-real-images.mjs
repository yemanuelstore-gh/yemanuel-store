// Real catalogue image generator.
// Reads scripts/catalogue-data/image-manifest.json (emitted by
// generate-real-catalogue.mjs) and writes one deterministic SVG per product
// to public/images/products/<slug>.svg, then rewrites INDEX.json.
//
// The SVGs are same-origin assets served from /public, so they pass the
// storefront image allowlist (src/lib/image-config.ts). They are rendered
// product tiles, not photographs: each tile uses a department palette, a
// deterministic decorative layout and the product/brand names.
...
const manifest = JSON.parse(readFileSync(`${root}/scripts/catalogue-data/image-manifest.json`, "utf8"));
...
// Clear previous placeholder/demo SVGs and the old index
for (const f of readdirSync(outDir)) {
  if (f.endsWith(".svg") || f === "INDEX.json") rmSync(join(outDir, f));
}
...
console.log(`Wrote ${files.length} product SVGs to ${outDir}`);
console.log(`INDEX.json updated (${(files.join(",").length / 1048576).toFixed(2)} MB of filenames)`);